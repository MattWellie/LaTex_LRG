...
class Parser:
        ...
        for exon in latex_dict['list_of_exons']:
            ...
            for char in sequence:
                ...
                (next_amino_number, amino_wait, codon_numbered,
                 amino_acid_counter) = self.decide_amino_number_string_character(char, amino_wait, codon_numbered,
                                                                                 amino_acid_counter)
    ...
    def run(self):
        ...
        for entry in self.transcriptdict['transcripts'].keys():
            outputfile = self.transcriptdict['genename'] + '_' + self.fileName.split('.')[0] + '_' + str(
                entry) + "_" + str(self.transcriptdict['pad'])
